...
class uvonode: 
    ...
    def process_image(self, cv_image):
        ...
        if (pts1 is not None and pts2 is not None):
            ...
            if self.pano is None: 
                self.pano =  cv2.copyMakeBorder(self.last_img, 100, 100, 100, 100, cv2.BORDER_CONSTANT, None, value=0)  # Black padding
                #self.pano = self.last_img
            ...
            ## add padding
            img_2 = cv2.copyMakeBorder(cv_image, 100, 100, 100, 100, cv2.BORDER_CONSTANT, None, value=[0,0,255,0])  # transparent padding
            #img_2 = cv_image
            ...
            #cv2.imshow("Translated and Overlaid Images", self.pano)
            ...
            ### below is stupid using stitching function

            # Append every nth image defined by pano_rate  
            # if self.count % self.pano_rate == 0:

            #     self.imgs.append(warp_dst)

            # Make panorama every n processed images
            # n = 2
            # if self.count % (self.pano_rate * n) == 0:
            #     result = cv2.addWeighted(self.imgs[0], 0.5, self.imgs[1], 0.5, 0)
                #cv2.imshow("Translated and Overlaid Images", result)

                #cv2.waitKey(3000)      
                # status, pano = self.stitcher.stitch(self.imgs)
                # self.imgs = []
                ...
                # filename =f"{package_path}/media/i2mage_{ self.count//(n*self.pano_rate):02d}.png"  
                # cv2.imwrite(filename, pano)
                # print( self.count//(n*self.pano_rate) ," - stitching completed successfully. Image saved!")
                # cv2.waitKey(10)
    ...
    def __init__ (self):
        ...
        #![stitching]
        self.stitcher = cv2.Stitcher.create(mode=1) # SCANS
        self.imgs = []
        self.count = 0
        self.pano_rate = 10
        self.M = None
